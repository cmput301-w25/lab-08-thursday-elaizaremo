...
public class MovieProviderTest {
    ...
    public void setUp() {
        ...
        MovieProviderTest.setInstanceForTesting(mockFirestore);
    ...
    */
    ...
    @Test
    public void testIsTitleUnique_whenTitleDNE() {
        when(mockFirestore.collection("movies").whereEqualTo("title", "Inception")).thenReturn(mockQuery);
        when(mockQuery.get()).thenReturn(mockTask);

        // Mock the Task behavior to return an empty QuerySnapshot (no movie found)
        when(mockTask.isSuccessful()).thenReturn(true);
        when(mockTask.getResult()).thenReturn(mockQuerySnapshot);
        when(mockQuerySnapshot.isEmpty()).thenReturn(true);
        boolean isUnique = movieProvider.isTitleUnique("Inception");
        assertTrue("Movie title should be unique.", isUnique);
